...
tt_notnow = false,         // Now is a bad time
...
tt_staticclick = false,    // static but permits mouseover of it
tt_borderpush = false,     // push off the bottom instead of automatically going to the top
tt_center = false,
tt_wait = false,
...
function tt_Htm(tt, t_id, txt)
{
	...
	t_y += 'left:0px;top:0px;width:' + (t_w+t_shw) + 'px;visibility:' + (tt_n4? 'hide' : 'hidden') + ';" onmouseover="tt_DontHide ();" onmouseout="tt_CheckHide ();">';
	...
	if (t_shw)
	{
		...
		else
		{
			...
			t_y += '<div id="' + t_id + 'S" style="position:relative;background:' + t_shc + ';left:' + t_spct + 'px;top:0px;width:' + (t_w-t_spct) + 'px;height:' + t_shw + 'px;overflow:hidden;' + t_opa + '"><\/div>';
		}
	}
	t_y += '<\/div>';
	return t_y;
		...
		var j = tags.length; while (j--)
		{
			if (typeof (t_tj = tags[j]).onmouseover == 'function' && t_tj.onmouseover.toString().indexOf(esc) != -1 && !tt_n6 || tt_n6 && (over = t_tj.getAttribute('onmouseover')) && over.indexOf(esc) != -1)
			{
				...
				t_tj.onmouseover = new Function('e',
					...
					(typeof t_tj.T_STATICCLICK != tt_u) + ','+
					(typeof t_tj.T_BORDERPUSH != tt_u) + ','+
					(typeof t_tj.T_CENTER != tt_u) +
					');'
				);
...
function tt_Show(t_e, t_id, t_above, t_delay, t_fix, t_left, t_offx, t_offy, t_static, t_sticky, t_staticclick, t_borderpush, t_center)
{
	if (tt_notnow) return;
	var new_obj = tt_GetDiv(t_id);
	if (tt_obj && tt_obj == new_obj && t_staticclick)
	{
		tt_DontHide ();
		return;
	}
	if (tt_obj) tt_ReallyHide();
	if (window.tt_rdl) window.clearTimeout (window.tt_rdl);
	window.tt_rdl = 0;
	...
	if (tt_obj)
	{
		...
		tt_static = t_static;
		tt_sticky = t_sticky;
		tt_staticclick = t_staticclick;
		tt_borderpush = t_borderpush;
		tt_center = t_center;
		tt_objW = tt_GetDivW();
		tt_objH = tt_GetDivH();
		tt_offX = t_center? (-tt_objW / 2) : (t_left? -(tt_objW+t_offx) : t_offx);
		tt_offY = t_offy;
		...
		window.tt_rdl = window.setTimeout(
			'window.tt_rdl = 0;' +
			'if (tt_sticky || tt_staticclick)'+
			'{'+
				'window.tt_upFunc = document.onmouseup || null;'+
				'if (document.captureEvents) document.captureEvents(Event.MOUSEUP);'+
				'document.onmouseup = new Function("window.setTimeout(\'tt_ReallyHide();\', 0xa);");'+
			'}'+
...
function tt_Hide()
{
	if (tt_staticclick && !window.tt_rdl) {
		window.tt_hidetimer = window.setTimeout ("tt_ReallyHide ();", 100);
	} else {
		tt_ReallyHide ();
	}
}
...
function tt_DontHide()
{
	if (window.tt_hidetimer) window.clearTimeout(window.tt_hidetimer);
	window.tt_hidetimer = 0;
	return;
}

function tt_ReallyHide()
{
	if (window.tt_obj)
	{
		if (window.tt_hidetimer) window.clearTimeout(window.tt_hidetimer);
		...
		//if (typeof window.tt_upFunc != tt_u) alert ('foo');
	}
}
